...
class MapMatching:
    ...
    def split_links_to_segments(links_gdf, link_id_col='id_link', link_direction_col=None):
        ...
        if link_direction_col: # Se i link hanno una direzione calcolo gli archi in direzione opposta
            ...
            links_gdf.loc[dir_reverse, 'geometry'] = links_gdf[dir_reverse].geometry.apply(lambda x: LineString(list(x.coords)[::-1]))
            
            double_direction = links_gdf[link_direction_col] == 0
            links_to_add_reverse = links_gdf[double_direction].copy()
            links_to_add_reverse.loc[:, 'geometry'] = links_to_add_reverse.geometry.apply(lambda x: LineString(list(x.coords)[::-1]))
    ...
    def match(self, gps_gdf, max_distance=50, max_angle=45, fcd_id_col='id_fcd', fcd_dir_col='heading', fcd_state_col='engine', all_matches=False):
        ...
        for idx, gps in gps_gdf.iterrows():
            ...
            possible_matches['alpha'] = possible_matches['azimuth'].apply(lambda x: MapMatching.angular_difference(x, gps[fcd_dir_col]))

            candidates = possible_matches[(possible_matches['dist'] <= max_distance) & (possible_matches['alpha'] < max_angle)].copy()

            if candidates.empty:
                #print("No candidates found for GPS trace with ID: ", gps[gps_id_col])
                continue

            candidates['p_alpha'] = candidates['alpha'].apply(lambda alpha: 0.1 if alpha <= 5 or gps[fcd_state_col] in [0, 2] else 0.1 * exp(-(alpha - 5) / 5))
            ...
            best_candidate['matched_pos'] = best_candidate['start_pos'] + relative_pos * (best_candidate['end_pos'] - best_candidate['start_pos'])
            ...
            if all_matches:
                candidates['matched_pos'] = candidates['start_pos'] + candidates.geometry.project(gps.geometry, normalized=True) * (candidates['end_pos'] - candidates['start_pos'])
                df_all_matches = candidates[['id_link', 'matched_pos', 'prob']].groupby('id_link').apply(
                    lambda x: pd.Series({
        ...
        if len(matched_results) == 0:
            if all_matches:                
                match = pd.DataFrame(columns=[fcd_id_col, 'mm_id_link', 'mm_pos', 'mm_seg_prob', 'mm_link_prob', "all_matches"])
            else:
