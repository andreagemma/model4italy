...
class Simulator:
    ...
    def spec_demand(self, flows):
        ...
        (preload, Inflow, R_mod_tot, Beta_out_tot, Beta_div_tot) = read_od.get_demand_profile(flows, self.sim_duration,
                                                                                              self.Ramps, self.Stars, self.Topology, self.SimuGraph)
    ...
    def agg_results(self, agg_int='5T'):
        ...
        if len(self.results) > 0:
            ...
            datevec = np.arange(start_date[0], end_date[0], timedelta(seconds=self.parameters.header.time_step_s))
            datevec_repeat = np.concatenate([np.tile(datevec[ix], len(links)) for ix in range(len(datevec))]).ravel()
            d = np.vstack([np.tile(links, self.sim_duration), q[:-1, :].flatten(), r[:-1, :].flatten(), v[:-1, :].flatten()]).T
