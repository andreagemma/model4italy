...
def trip_dijkstra(graph: AbstractGraph,
    ...
    def calc_distance_fcd(line1:LineString, trips_points:list[Point]):
        if line1 is None:
            return float("inf")
        if not trips_points:
            return 0
        dist = line1.distance
        dists = [dist(p) for p in trips_points]
        dists = [d for d in dists if d >40]
        if not dists:
            return 0
        return min(dists)
    ...
    vol = 50 / 60 * 1000 # 833 m/min : costo di in metri di un minuto da usare per bilanciare costo distanza e costo tempo
    w_cost = 5* vol # peso del tempo
    w_distance_fcd = 10 # costo per i path + lontani di 50m dai dati fcd
    pq = []
    ...
    for l in graph.get_fws(source):
        ...
        initial_cost += w_distance_fcd * calc_distance_fcd(paths_geoms[l_idx], trip_points)
    ...
    while pq:
        ...
        for next_l in graph.get_fws(current_l_j):
            ...
            new_cost += w_cost * next_l["t0"]
            new_cost_fcd = w_distance_fcd * calc_max_distance_fcd(new_line, trip_points)
            ...
            if new_cost < paths_costs.get(next_l_idx, float("inf")):
                push(pq, (new_cost+new_cost_fcd, new_cost, next_l_idx, next_l_i, next_l_j, next_l))                        
    ...
    print(calc_distance_fcd(paths_geoms[current_l_idx], trip_points))
