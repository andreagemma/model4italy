...
from ..graphs import (
    DynamicGraph as Graph,
    DynamicLink as Link,
    DynamicNode as Node,
    DynamicTurn as Turn,
    DynamicGraphElement as GraphElement,
)
...
class AssignmentModel(BaseM4IModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.save_agg_results_stats = (
            save_agg_results_stats
            and self.writer.has_write_agg_results_stats()
            and self.simulator is not None
            and self.save_results
        )
        self.save_trace_results = (
            save_trace_results
            and self.writer.has_write_trace_results()
            and self.simulator is not None
            and self.save_results
        )
        self.save_signal_results = (
            save_signal_results
            and self.writer.has_write_signal_results()
            and self.simulator is not None
            and self.save_results
        )
    ...
    def run(self):
        ...
        self.log.info(
            f"""Simulation Parameters: start: {start} end: {end} duration: {duration} time_slice: {self.global_time_slice} delta_t: {self.delta_t}"""
        )

        if len(self.global_intervals) > 1:
            self.log.info(
                f"Simulation will be splitted into {len(self.global_intervals)} intervals: {self.global_intervals}"
            )
        ...
        try:
            if self.save_paths:
                ...
                if paths is None or paths.empty:
                    ...
                    return
                mode = None if self.interval == 0 else "a"
                saved = self.writer.write_paths(
                    paths,
                    mode=mode,
                    crs=self.loader.ini.CRS,
                    first_query=self.interval == 0,
                )
                paths = None
                if saved:
            ...
            if self.save_state_paths:
                ...
                for t in range(self.real_time_start, self.real_time_end, self.delta_t):
                    ...
                    self.state_manager.write_state(
                        paths,
                        "paths",
                        partition=f"day_type={self.parser.get('day_type_simulation')}/t={t}",
                    )
                paths = None
    ...
    def write_agg_results_stats(self):
        try:
            if self.save_agg_results_stats:
                ...
                if df is not None and not df.empty:
                    mode = None if self.interval == 0 else "a"
                    saved = self.writer.write_agg_results_stats(
                        df,
                        mode=mode,
                        crs=self.loader.ini.CRS,
                        first_query=self.interval == 0,
        ...
        try:
            if self.save_trace_results:
                ...
                if df is not None and not df.empty:
                    ...
                    saved = self.writer.write_trace_results(
                        df,
                        mode=mode,
                        crs=self.loader.ini.CRS,
                        first_query=self.interval == 0,
        ...
        try:
            if self.save_signal_results:
                ...
                if df is not None and not df.empty:
                    ...
                    mode = None if self.interval == 0 else "a"
                    df["t"] = ds_t.astype("Float32")
                    saved = self.writer.write_signal_results(
                        df,
                        mode=mode,
                        crs=self.loader.ini.CRS,
                        first_query=self.interval == 0,
            ...
            if self.save_state_ass_matrix:
                ...
                for t_enter, mat in self.ass_matrix.get_all_matrix_by_tenter():
                    self.state_manager.write_state(
                        ...
                        "ass_matrix",
                        mode="w",
                        partition=f"day_type={self.parser.get('day_type_simulation')}/t_enter={t_enter}",
                    )
                mat = None
                self.log.info("Saved state")
        except Exception as e:
    ...
    def run_internal_step(self):
        ...
        self.G.resize_attributes(
            ...
            offset=self.current_i_start * self.delta_t,
        )
                ...
                for t_start, t in enumerate(
                    ...
                    if df_paths is not None and not df_paths.empty:
                        ...
                        df_paths = df_paths[
                            ...
                            & df_paths["target"].isin(self.loader.destinations)
                        ]
    ...
    def calculate_paths(self):
        self.task_step_done(
            f"{min2hhmm(self.current_time_start)}-{min2hhmm(self.current_time_end)} - Iteration: {self.iteration}/{self.max_ite} - Calculating paths"
        )
        n_cpu = self.loader.ini.MSA_SPP_NUMCPUS

        ret = KPathList()

        ret = SPP.multiple_paths(
            graph=self.G,
    ...
    def update_performance(self):
        self.task_step_done(
            f"{min2hhmm(self.current_time_start)}-{min2hhmm(self.current_time_end)} - Iteration: {self.iteration}/{self.max_ite} - Updating performance"
        )
        ...
        if self.simulator:
            ...
            if isinstance(self.simulator, MicroSimulator):
                ...
                if update_costs:
                    self.update_paths_costs(
                        update_nodes=self.nodes_cost is not None,
                        update_links=self.links_cost is not None,
    ...
    def update_paths_costs(
        ...
        for path in self.m_paths.all_paths():
            ...
            for i, l_idx in enumerate(path.get_links()):
                ...
                if update_nodes:
                    cost += self.G.get_node(l["i"]).get_value(
                        name=self.nodes_cost,
                        t=t,
                        in_link=prev_l,
                        out_link=l,
                        graph=self.G,
    ...
    def calculate_ass_matrix(self):
        ...
        def fn_calc_mat_ass(
            ...
            for (source, target, t_start, mode), paths in tasks:
                for path in paths:
                    ...
                    for t, l_idx in zip(
                        costs, links
                    ):  # [t for t in zip(idxs, links) if t[1] in self.matrix_ass.detectors]:
        ...
        n_workers = 1
        if n_workers > 1:
            for params in Parallel.execute(
                fn_calc_mat_ass,
                tasks,
                n_workers=n_workers,
                eq_factors=self.eq_factors,
                G=self.G,
                links_cost=self.links_cost,
                nodes_cost=self.nodes_cost,
                turns_cost=self.turns_cost,
                current_time_start=self.current_time_start,
                OD=self.OD,
                detectors=self.ass_matrix.detectors,
            ):
    ...
    def get_aggregated_results_dataframe(self):
        ...
        # return results
        res = self.simulator.agg_results(
            self.global_t_start,
            self.global_t_end,
            agg_int=self.loader.ini.OUTPUT_AGG_INT,
        )
