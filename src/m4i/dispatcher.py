    ...
    def __init__(self, params, ini, options: dict = None, execution: Execution = None):
        ...
        if self.execution_id is None:
            try:
                ...
                if appname:
                    raise Exception(
                        f"Error creating execution in database. Try to initialize the database with {appname} init_db"
                    ) from ex
                else:
                    parent_module_name = __name__.split(".")[0]
                    raise Exception(
                        f"Error creating execution in database. Try to initialize the database with '{parent_module_name}.init_db()' command"
                    ) from ex
        ...
        if self.parser.ini.PARALLEL_USE:
            self.log.debug(
                f"Initializing parallel engine {self.parser.ini.PARALLEL_ENGINE} with {self.parser.ini.PARALLEL_NUMCPUS} cpus..."
            )
    ...
    def run(self) -> "Dispatcher":
        try:
            ...
            if self.execution_id is None:
                ...
                self.log.info(
                    f"Execution ({self.execution_id}) finished in %.2f seconds",
            ...
            if self.execution_id is None:
                self.log.info(
                    "Execution terminated abnormally in %.2f seconds",
                    time.time() - self.t_start,
                )
            else:
