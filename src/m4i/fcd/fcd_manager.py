...
class FCDManager(BaseM4IModel):
    ...
    def load_fcd_by_timestamp(self, t_start: datetime, t_end: datetime) -> gpd.GeoDataFrame:
        ...
        df_fcd = self.loader.load(
            ...
            filters=[
                [
                    ["timestamp", ">=", t_start.strftime("%Y-%m-%d %H:%M:%S%z")],
                    ["timestamp", "<", t_end.strftime("%Y-%m-%d %H:%M:%S%z")],
                ]
            ],
            dtype=dtype,
        )
    ...
    def build_trips(
        ...
        signal_break_max_dt: Optional[float] = None,
        signal_break_dt: Optional[float] = None,
        signal_break_v: Optional[float] = None,
        stop_o_ds: Optional[float] = None,
        stop_d_ds: Optional[float] = None,
        signal_cont_dt: Optional[float] = None,
        signal_cont_v: Optional[float] = None,
        ...
        for df_trips, df_fcd_truncated in Parallel.execute(
            process_single_vehicle,
            tasks,
            n_workers=1 if DEBUG else self.parser.ini.FCD_TRIPS_CPUS,
        ):
            if df_trips is not None and len(df_trips) > 0:
        ...
        if ret_df_trips is not None:
            ret_df_trips = gpd.GeoDataFrame(
                ret_df_trips,
                crs=self.crs_calc,
                geometry=shapely.from_wkt(ret_df_trips["geometry"]),
            )
        if ret_df_fcd_truncated is not None:
            ret_df_fcd_truncated = gpd.GeoDataFrame(
                ret_df_fcd_truncated,
                crs=self.crs_calc,
                geometry=shapely.from_wkt(ret_df_fcd_truncated["geometry"]),
            )
        return ret_df_trips, ret_df_fcd_truncated
    ...
    def _process_single_vehicle(
        params: namedtuple, vehicle_df: pd.DataFrame, DEBUG: bool = False
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for i, fcd in enumerate(fcds):
            ...
            if fcd_prev is not None:
                ...
                if (
                    fcd_prev.ts == fcd.ts
                ):  # and fcd_prev.progr == fcd.progr and fcd_prev.x == fcd.x and fcd_prev.y == fcd.y:  # delete 2 samples with same timestamp
                ...
                if fcd_prev.id_veh != fcd.id_veh:  # changed vehicle
                    if DEBUG:
                        print(
                            "new_trip: changed vehicle: %s -> %s",
                            fcd_prev.id_veh,
                            fcd.id_veh,
                        )
                        print(
                            "Start procedure to reconstruction of trajectory for id_vehicle '%s' id %s",
                            fcd.id_veh,
                            fcd.id_fcd,
                        )
                    ...
                    if DEBUG:
                        print(
                            "new_trip: max-t signal interruption  (t=%d, v=%.2f, d=%.1f) in %s",
                            tt,
                            v_euc,
                            euc_dist,
                            fcd.id_fcd,
                    ...
                    tt > params.signal_break_dt and v_euc < params.signal_break_v
                ):  # signal interruption and very low speed
                    if DEBUG:
                        print(
                            "new_trip: signal interruption (t=%d, v=%.2f, d=%.1f) in %s",
                            tt,
                            v_euc,
                            euc_dist,
                            fcd.id_fcd,
                    ...
                    if tt2 > 0:
                        ...
                        # exclude first fcd
                            ...
                            if DEBUG:
                                print(
                                    "remove first fcd: max_v3 (v-ab=%.2f, v-bc=%.2f) in %s",
                                    v_euc,
                                    v_euc2,
                                    fcd.id_fcd,
                                )
            ...
            if not new_trip:  # if no new_trip add fcd to current list
                ...
                if params.remove_stops:
                    ...
                    if len(trip_fcds) >= 2:
                        ...
                        while len(trip_fcds) >= 2:
                            ...
                            euc_dist = hypot(o.x - trip_fcds[1].x, o.y - trip_fcds[1].y)
                    ...
                    if n > len(trip_fcds):
                        if DEBUG:
                            print(
                                "removed %s records at start in trip %s->%s",
                                n - len(trip_fcds),
                                id_trip,
                                trip_fcds[0].id_fcd,
                            )
                    ...
                    if n > len(trip_fcds):
                        if DEBUG:
                            print(
                                "removed %s records at end in trip %s",
                                n - len(trip_fcds),
                                id_trip,
                            )
                        pass
                ...
                if error_in_max_distance:
                    ...
                    if removed > 0:
                        if DEBUG:
                            print(
                                "removed %s records for max_distance in trip %s",
                                removed,
                                id_trip,
                            )
            ...
            df = df.astype(
                {
                    "id_trip": "Int64",
                    "id_veh": str,
                    "dt_o": f"datetime64[ns, {tzname}]",
                    "dt_d": f"datetime64[ns, {tzname}]",
                    "tt": np.float64,
                    "dist": np.float64,
                    "avg_speed": np.float64,
                    "id_fcds": object,
                    "progr_o": np.float64,
                    "id_prev": "Int64",
                    "id_next": "Int64",
    ...
    def map_matching_fcd(
        self,
        df_fcd: gpd.GeoDataFrame,
        df_links,
        links_id_col,
        links_direction_col,
        segments_gdf=None,
        ...
        self.mm = MapMatching(
            ...
            links_direction_col=links_direction_col,
    ...
    def match_fcd(
        self, fcd_data: pd.DataFrame, old_fcd_data: pd.DataFrame
        ...
        if self.df_trips is not None:
            ...
            merged = df_trips.merge(
                self.df_trips.set_index("id_trip", drop=True),
                on="id_trip",
                how="left",
                suffixes=("_new", "_old"),
            )
