    ...
    def _apply_inplace_operation(self, b, op) -> None:
        ...
        if isinstance(b, DynamicValueAttribute):
            dict.__setitem__(
                self,
                "value",
                op(dict.__getitem__(self, "value"), dict.__getitem__(b, "value")),
            )
...
class DynamicTimeArrayAttribute(DynamicAttribute, Generic[T]):
    ...
    def _apply_inplace_operation(self, b, operation) -> None:
        ...
        if (
            isinstance(b, DynamicTimeArrayAttribute)
            and dict.__getitem__(b, "num_intervals") == num_intervals
            and dict.__getitem__(b, "delta_t") == delta_t
        ):
        ...
        **kwargs,
        ...
        if callable(value):
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid value '{value}'. Expected 'module:function_name', e.g., function.vdf:bpr to use bpr function in function.vdf module"
                )
        ...
        if isinstance(b, DynamicCallableAttribute):
            ...
            if t is None:
                ...
                dict.__setitem__(
                    self,
                    "fn",
                    lambda **kw: fn(**kw) if kw["t"] != t else op(fn(**kw), fnb(**kw)),
                )
        elif isinstance(b, DynamicAttribute):
            b = b.copy()
            if t is None:
            ...
            if t is None:
                ...
                dict.__setitem__(
                    self,
                    "fn",
                    lambda **kw: fn(**kw) if kw["t"] != t else op(fn(**kw), b(**kw)),
                )
        else:
            if t is None:
                ...
                dict.__setitem__(
                    self,
                    "fn",
                    lambda **kw: fn(**kw) if kw["t"] != t else op(fn(**kw), b),
                )
