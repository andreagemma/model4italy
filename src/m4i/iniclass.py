...
class IniClass:
    ...
    def __init__(
        ...
        # if self.LOG_USE:
        #    logging.debug("File %s caricato", os.path.abspath(ini_file))
    ...
    def load_parameters(self):
        ...
        self.LOG_EXECUTION_FORMAT = self.config_reader.get(
            ...
            "LOGGING",
            "%(execution_id)s - %(asctime)s | %(last_elapsed).2f/%(elapsed).2fs | %(levelname)s | %(name)s | %(message)s",
        )
        self.LOG_FORMAT = self.config_reader.get(
            "LOG_FORMAT",
            "LOGGING",
        ...
        self.RANDOM_SEED = self.config_reader.getint(
            "RANDOM_SEED", "GENERAL", 0
        ...
        self.OD_ESTIMATION_LAMBDA_UB = self.config_reader.getfloat(
            "OD_ESTIMATION_LAMBDA_UB", "OD_ESTIMATION", 0.05
        )  # Estremo superiore
        self.OD_ESTIMATION_EPS = self.config_reader.getfloat(
            "OD_ESTIMATION_EPS", "OD_ESTIMATION", 0.000001
        )  # Soglia di arresto dell'ottimizzazione monodimensionale
        self.OD_ESTIMATION_EPS2 = self.config_reader.getfloat(
            "OD_ESTIMATION_EPS2", "OD_ESTIMATION", 10
        )  # Soglia di arresto dell'ottimizzazione monodimensionale
        self.OD_ESTIMATION_SA = self.config_reader.getfloat(
            "OD_ESTIMATION_SA", "OD_ESTIMATION", 0.362
        )  # segmento minore della sezione aurea

        # FCD_SERVER
        self.FCD_SERVER_FCD_HORIZON = self.config_reader.getint(
            "FCD_SERVER_FCD_HORIZON", "FCD_SERVER", 60
        )  # orario di previsione in minuti
        self.FCD_SERVER_FCD_TIMESLICE = self.config_reader.getint(
            "FCD_SERVER_FCD_TIMESLICE", "FCD_SERVER", 15
        )  # numero massimo di iterazioni per la previsione
        self.FCD_SERVER_FCD_TIMESLICE_OFFLINE = self.config_reader.getint(
            "FCD_SERVER_FCD_TIMESLICE_OFFLINE", "FCD_SERVER", 120
        )  # numero di iterazioni per la previsione offline
        self.FCD_SERVER_SHARE_DATA_ONLINE = self.config_reader.getboolean(
            "FCD_SERVER_SHARE_DATA_ONLINE", "FCD_SERVER", True
        )  # esporta i dati FCD su IPC
        self.FCD_SERVER_WRITE_OUTPUT = self.config_reader.getboolean(
            "FCD_SERVER_WRITE_OUTPUT", "FCD_SERVER", False
        )  # esporta i dati FCD su DB
        self.FCD_SERVER_MAP_MATCHING = self.config_reader.getboolean(
            "FCD_SERVER_MAP_MATCHING", "FCD_SERVER", True
        )  # abilita il map matching
        self.FCD_SERVER_ROUTING = self.config_reader.getboolean(
            "FCD_SERVER_ROUTING", "FCD_SERVER", True
        )  # abilita il routing
        self.FCD_SERVER_TRIPS = self.config_reader.getboolean(
            "FCD_SERVER_TRIPS", "FCD_SERVER", True
        )  # abilita la generazione dei viaggi
        self.FCD_SERVER_UPDATE_SPEED = self.config_reader.getboolean(
            "FCD_SERVER_UPDATE_SPEED", "FCD_SERVER", True
        )  # calcola velocità media dei veicoli
        self.FCD_SERVER_WRITE_STATE = self.config_reader.getboolean(
            "FCD_SERVER_WRITE_STATE", "FCD_SERVER", True
        )  # salva lo stato della simulazione ad ogni timeslice
        self.FCD_SERVER_SAVE_DELAY = self.config_reader.getint(
            "FCD_SERVER_SAVE_DELAY", "FCD_SERVER", 0
        )  # ritardo del server FCD in minuti per avviare il salvataggio dei dati (utile per recovery da crash)
        self.FCD_SERVER_SAVE_START = self.config_reader.get(
            "FCD_SERVER_SAVE_START", "FCD_SERVER", None
        )  # istante di inizio salvataggio dati (None per salvare dall'inizio) - sovrascrive FCD_SERVER_SAVE_DELAY se presente
        self.FCD_SPEED_AGGREGATION_INTERVAL = self.config_reader.getint(
            "FCD_SPEED_AGGREGATION_INTERVAL", "FCD_SERVER", 15
        )  # intervallo di aggregazione della velocità in secondi
        self.FCD_SERVER_RECOVERY_MODE = self.config_reader.getboolean(
            "FCD_SERVER_RECOVERY_MODE", "FCD_SERVER", False
        )  # è o meno in modalità recovery (True per modalità recovery, False per modalità normale)

        self.FCD_MAP_MATCHING_CPUS = self.config_reader.getint(
            "FCD_MAP_MATCHING_CPUS", "FCD_MAP_MATCHING", 1
        )  # numero di processi per il map matching
        self.FCD_MAP_MATCHING_MAX_DISTANCE = self.config_reader.getfloat(
            "FCD_MAP_MATCHING_MAX_DISTANCE", "FCD_MAP_MATCHING", 50
        )  # distanza massima per il map matching in metri
        self.FCD_MAP_MATCHING_MAX_ANGLE = self.config_reader.getfloat(
        ...
        self.FCD_ROUTING_END_TO_ZONE = self.config_reader.getboolean(
            "FCD_ROUTING_END_TO_ZONE", "FCD_ROUTING", False
        )  # se True il path termina nella zona di arrivo
        self.FCD_ROUTING_AGGRATION_INTERVAL = self.config_reader.getint(
            "FCD_ROUTING_AGGRATION_INTERVAL", "FCD_ROUTING", 15
        )  # intervallo di aggregazione in secondi
        self.FCD_ROUTING_CLUSTERING = self.config_reader.getboolean(
            "FCD_ROUTING_CLUSTERING", "FCD_ROUTING", True
        )  # se True viene eseguito il clustering dei percorsi FCD
        self.FCD_ROUTING_CLUSTERING_EPS = self.config_reader.getfloat(
            "FCD_ROUTING_CLUSTERING_EPS", "FCD_ROUTING", 100
        )  # eps per il clustering dei percorsi FCD in metri

        self.FCD_TRIPS_CPUS = self.config_reader.getint(
            "FCD_TRIPS_CPUS", "FCD_TRIPS", 1
        )  # numero di processi per la generazione dei viaggi
        self.FCD_TRIPS_SIGNAL_BREAK_MAX_DT = self.config_reader.getint(
            "FCD_TRIPS_SIGNAL_BREAK_MAX_DT", "FCD_TRIPS", 900
        )  # max time for signal break in seconds
        self.FCD_TRIPS_SIGNAL_BREAK_DT = self.config_reader.getint(
            "FCD_TRIPS_SIGNAL_BREAK_DT", "FCD_TRIPS", 300
        )  # time for signal break in seconds
        self.FCD_TRIPS_SIGNAL_BREAK_V = self.config_reader.getfloat(
            "FCD_TRIPS_SIGNAL_BREAK_V", "FCD_TRIPS", 0.5555555555555556
        )  # speed for signal break in m/s
        self.FCD_TRIPS_STOP_O_DS = self.config_reader.getfloat(
            "FCD_TRIPS_STOP_O_DS", "FCD_TRIPS", 50
        )  # distance for stop at origin in meters
        self.FCD_TRIPS_STOP_D_DS = self.config_reader.getfloat(
            "FCD_TRIPS_STOP_D_DS", "FCD_TRIPS", 50
        )  # distance for stop at destination in meters
        self.FCD_TRIPS_SIGNAL_CONT_DT = self.config_reader.getint(
            "FCD_TRIPS_SIGNAL_CONT_DT", "FCD_TRIPS", 600
        )  # time for signal continuation in seconds
        self.FCD_TRIPS_SIGNAL_CONT_V = self.config_reader.getfloat(
            "FCD_TRIPS_SIGNAL_CONT_V", "FCD_TRIPS", 0.1388888888888889
        )  # speed for signal continuation in m/s
        ...
        )  # min length of trip in meters
        self.FCD_TRIPS_MIN_TIME = self.config_reader.getint(
            "FCD_TRIPS_MIN_TIME", "FCD_TRIPS", 1
        )  # min time of trip in seconds
        self.FCD_TRIPS_REMOVE_STOPS = self.config_reader.getboolean(
            "FCD_TRIPS_REMOVE_STOPS", "FCD_TRIPS", False
        )  # remove stops in trips
        self.FCD_TRIPS_MAX_DISTANCE_BETWEEN_DATA = self.config_reader.getfloat(
            "FCD_TRIPS_MAX_DISTANCE_BETWEEN_DATA", "FCD_TRIPS", 20000
        )  # max distance between data in meters
        self.FCD_TRIPS_MAX_DELTA_PROGR = self.config_reader.getfloat(
            "FCD_TRIPS_MAX_DELTA_PROGR", "FCD_TRIPS", 20000
        ...
        self.OUTPUT_IND_RES = self.config_reader.getboolean(
            "OUTPUT_IND_RES", "OUTPUT", False
        )  # Indica se vengono tracciati i risultati individuali
        self.MONITORED_VEH = self.config_reader.getint(
            "MONITORED_VEH", "OUTPUT", 30
        )  # Percentuale di veicoli da monitorare
        self.OUTPUT_STATS = self.config_reader.get(
            "OUTPUT_STATS", "OUTPUT", True
        )  # Indica se vengono salvate le statistich
        ...
        self.IPC_COMPRESSION_LEVEL = self.config_reader.getint(
            "IPC_COMPRESSION_LEVEL", "IPC", 5
        ...
        self.GRAPH_PENALTY_RIGHT_ELBOW = self.config_reader.getint(
            "GRAPH_PENALTY_RIGHT_ELBOW", "GRAPH", 1
        )  # in seconds
        self.GRAPH_PENALTY_RIGHT_SHARP = self.config_reader.getint(
            "GRAPH_PENALTY_RIGHT_SHARP", "GRAPH", 1
        )  # in seconds
