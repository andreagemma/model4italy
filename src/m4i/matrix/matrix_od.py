...
class MatrixOD:
    ...
    def copy(self, copy_data: bool = True) -> "MatrixOD":
        return MatrixOD(
            self.rows,
            self.cols,
            init=self.mat.copy() if copy_data else self.mat,
            copy=copy_data,
        )
    ...
    def read_df(
        ...
        grouped = (
            df[[o_field, d_field, value_field]]
            .rename(columns={o_field: "o", d_field: "d", value_field: "value"})
            .groupby("o", group_keys=False)
            .apply(lambda x: dict(zip(x["d"], x["value"])), include_groups=False)
            .to_dict()
        )
        if od is not None:
                ...
                data.append(
                    {
                        o_field: o_key,
                        d_field: d_key,
                        value_field: self.mat[o_index, d_index],
                    }
                )
