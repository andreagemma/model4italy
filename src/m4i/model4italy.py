    ...
    try:
        ...
        if len(sys.argv) > 1 and not kwargs:
            ...
            parser.add_argument(
                ...
                "--params",
                default="params.json",
                help="JSON file with parameters (default: params.json)",
            )
            parser.add_argument(
                "-d",
                "--params_data",
                help="JSON file with data parameters (default: params_data.json)",
            )
            parser.add_argument(
                "-c",
                "--config",
            ...
            parser.add_argument(
                "-m",
                "--monitor",
                help="Folder of results of monitor process. If not specified, the process is not monitored",
            )
            parser.add_argument(
                "-O",
                "--option",
