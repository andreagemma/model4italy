...
class PathsCalculation(OP):
    def __init__(self, loader: Loader, writer: Writer, **kwargs):
        ...
        self.G = self.loader.load_graph().resize_attributes(
            new_total_time=self.parser.get("total_time", default=self.ini.DELTA_T)
        )
    ...
    def run(self):
        ...
        paths = SPP.multiple_paths(
            self.G,
            origins=self.origin_nodes["id"].tolist(),
            targets=self.destination_nodes["id"].tolist(),
