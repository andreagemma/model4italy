            ...
            if pathg.is_file():
                ...
                if filter:
                    df = (
                        ...
                        .sql(f"select * from self {sql_filters}")
                        .collect()
                        .to_pandas()
                    )
                else:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if ext == ".csv":
                ...
                if index:
                    df = df.reset_index()
                if partition_cols:
                    BaseDriver.write_partitioned(
                        df,
                        file=pathg,
                        partition_cols=partition_cols,
                        support_append=True,
                        fn_save=lambda x, f: gpd.GeoDataFrame.to_file(
                            x, Path(f).as_posix(), driver="ESRI Shapefile", mode="a"
                        ),
                    )
                else:
                ...
                if partition_cols:
                    BaseDriver.write_partitioned(
                        ...
                        fn_save=lambda x, f: gpd.GeoDataFrame.to_file(
                            ...
                            driver="GPKG",
                            mode="a",
                            layer=kwargs.get("layer", Path(f).stem),
                        ),
                    )
                else:
...
def write_geoparquet(
    ...
    if mode == "w":
        ...
        if missing_parts:
            raise ValueError(
                f"Le colonne di partizione {missing_parts} non sono presenti nel DataFrame risultante. "
