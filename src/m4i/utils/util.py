...
def to_datetime_auto(
    t: Union[str, int, float, datetime, time, date],
            ...
            try:
                ...
                return refine(
                    parser.parse(
                        t,
                        fuzzy=True,
                        default=datetime.combine(date_default, time_default),
                    )
                )
...
def to_timedelta_auto(
    ...
    if isinstance(t, str) and t.strip():
        ...
        return timedelta(
            hours=t.hour - base_date.hour,
            minutes=t.minute - base_date.minute,
            seconds=t.second - base_date.second,
            microseconds=t.microsecond - base_date.microsecond,
        )


def memory_usage(
    process_filter: Optional[Union[Iterable[Union[str, int]], int, str]] = [
        "python",
        "ray",
        "dask",
    ],
...
def export_dataframe(df, file_path, mode="w", **kwargs):
    ...
    export_methods_gpd = {
        ...
        ".geoparquet": lambda df, x, **kwargs: to_geoparquet(
            df, x, partition_cols=partition_cols, append=append, index=index
        ),
        ".gpkg": lambda df, x, **kwargs: (
            df.to_file(
                x,
                driver="GPKG",
                layer=kwargs.get("layer", os.path.basename(x)),
                index=index,
                mode=mode,
            )
            if hasattr(df, "to_file")
            else None
        ),
    }
...
def inner_filter_to_query_expression(
    ...
    if isinstance(filters, (tuple, list)):
        ...
        raise ValueError(
            "Invalid filter format. The inner filter must be a tuple with 3 elements (column,operator,value)."
        )
        ...
        import_methods_gpd.update(
            {
                ...
                ".gpkg": lambda x: geopandas.read_file(
                    x, driver="GPKG", layer=geopandas.list_layers(x)["name"].iloc[0]
                ),
...
def pandas_query_to_sql(expr: str, *, return_with_where: bool = True) -> str:
    ...
    s = re.sub(
        r'(\b\w+|"[^"]+")\s*\.\s*isnull\s*\(\s*\)',
        r"\1 IS NULL",
        s,
        flags=re.IGNORECASE,
    )
    s = re.sub(
        r'(\b\w+|"[^"]+")\s*\.\s*notnull\s*\(\s*\)',
        r"\1 IS NOT NULL",
        s,
        flags=re.IGNORECASE,
    )
    ...
    s = re.sub(
        r'(\b\w+|"[^"]+")\s*\.\s*str\s*\.\s*contains\s*\(\s*([\'"])(.*?)\2\s*(?:,[^)]*)?\)',
        _contains_to_like,
        s,
        flags=re.IGNORECASE,
    )
    ...
    s = re.sub(
        r'(\b\w+|"[^"]+")\s*\.\s*str\s*\.\s*startswith\s*\(\s*([\'"])(.*?)\2\s*\)',
        _starts_to_like,
        s,
        flags=re.IGNORECASE,
    )
    ...
    s = re.sub(
        r'(\b\w+|"[^"]+")\s*\.\s*str\s*\.\s*endswith\s*\(\s*([\'"])(.*?)\2\s*\)',
        _ends_to_like,
        s,
        flags=re.IGNORECASE,
    )
