...
class AttributeFramework:  
    def __init__(self):
        from m4i.graphs import DynamicValueAttribute, DynamicTimeArrayAttribute, DynamicCallableAttribute, DynamicAttribute
        system_info = f"System: {platform.node()}\n" f"CPU: {platform.processor()}\n" f"RAM: {psutil.virtual_memory().total / (1024 ** 3):.2f} GB\n"
        print(system_info)
        ...
        self.callable_attr = DynamicCallableAttribute(lambda **kw: kw["t"] * 2, total_time=10, delta_t=2)
...
def test_time_array_attribute_operations():
    ...
    tmp /= 2
    assert tmp.get_items() == [(0, 16), (2, 16), (4, 5), (6, 16), (8, 16), (10, 16), (12, 16), (14, 16), (16, 16), (18, 16)]
    ...
    assert array_attr.get_items() == [(0, 16), (1, 16), (2, 16), (3, 16), (4, 5), (5, 5), (6, 16), (7, 16), (8, 16), (9, 16), (10, 16), (11, 16)]

    tmp = array_attr + self.array_attr.copy()
    assert isinstance(tmp, DynamicTimeArrayAttribute)
    assert list(tmp.get_items()) == [(0, 21), (1, 21), (2, 21), (3, 21), (4, 10), (5, 10), (6, 21), (7, 21), (8, 21), (9, 21), (10, 21), (11, 21)]

    tmp = array_attr + self.callable_attr.copy()
    assert isinstance(tmp, DynamicTimeArrayAttribute)
    assert list(tmp.get_items()) == [(0, 16), (1, 16 + 2), (2, 16 + 4), (3, 16 + 6), (4, 5 + 8), (5, 5 + 10), (6, 16 + 12), (7, 16 + 14), (8, 16 + 16), (9, 16 + 18), (10, 16 + 20), (11, 16 + 22)]

    array_attr += 1
    assert array_attr.get_items() == [(0, 17), (1, 17), (2, 17), (3, 17), (4, 6), (5, 6), (6, 17), (7, 17), (8, 17), (9, 17), (10, 17), (11, 17)]
...
def test_callable_attribute_operations():
    ...
    assert callable_attr.get_items() == [(0, 4 + 0), (1, 4 + 2), (2, 4 + 4), (3, 4 + 6), (4, 4 + 8)]
...
def test_callable_attribute_different_functions():
    ...
    # Linear function
    linear_attr = DynamicCallableAttribute(lambda **kw: kw["t"] + 5, total_time=8, delta_t=2)
    assert list(linear_attr.get_values()) == [5, 7, 9, 11]

    # Quadratic function
    quad_attr = DynamicCallableAttribute(lambda **kw: kw["t"] ** 2, total_time=6, delta_t=2)
    assert list(quad_attr.get_values()) == [0, 4, 16]

    # Trigonometric function (using integer approximation)
    trig_attr = DynamicCallableAttribute(lambda **kw: int(kw["t"] % 4), total_time=8, delta_t=2)
...
def test_mixed_attribute_arithmetic_chains():
    ...
    callable_attr = DynamicCallableAttribute(lambda **kw: kw["t"], total_time=4, delta_t=2)
