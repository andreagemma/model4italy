...
def test_io_dataframe_export_import(tmp_path, ext):
    ...
    io.export_dataframe(gdf, path=str(filename), mode="w", partitionby=None, template="{filename}-{partition}-{i}", index=False)
    ...
    io.export_dataframe(gdf2, path=str(filename), mode="a", partitionby=None, template="{filename}-{partition}-{i}", index=False)
    assert os.path.exists(filename), "Export failed"
